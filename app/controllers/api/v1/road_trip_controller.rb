class Api::V1::RoadTripController < ApplicationController
  ...
  def create
    ...
    time = MapqFacade.route(body[:origin], body[:destination])
  ...
  def authenticate_user_with_token
    body = JSON.parse(request.raw_post, symbolize_names: true)
    @user ||= ApiKey.find_by(token: body[:api_key])
  end
  ...
  def get_weather_poro(location, trip_time)
    lat, lon = MapqFacade.coordinates(location)
    WeatherFacade.destination_weather(lat, lon, trip_time)
  end
end